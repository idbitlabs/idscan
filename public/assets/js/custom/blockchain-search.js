function blockChainSearchInit({
    ...
    function redirectBasedOnGroupId({
        searchkey = ''
    }) {
        let currtypeval = getTypeval({
            searchkey
        });
    ...
    let autocomplete = new Autocomplete(id, {
        ...
        onResults: ({
            ...
            return matches === 0 ? template : matches.map((el, index, array) => {
                let group = (isMainPanel || isAdvancedFilterSearch) && el[attributes.group] !== array[index - 1] ? .[attributes.group] ? `<div class="search-category-${el[attributes.group].toLowerCase().replace(/ /g,'-').replace('-('+siteToken.toLowerCase()+'-20)','').replace('-('+siteToken.toLowerCase()+'-721-&amp;-1155)','')} ${classGroup}" style="font-size: 0.65rem;"><span>${el[attributes.group]}</span></div>` : "";
                let img_url = '';
                let altText = '';
